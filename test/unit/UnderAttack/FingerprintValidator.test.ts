...
describe('FingerprintValidator', () => {
    ...
    beforeEach(() => {
        fingerprintValidator = new FingerprintValidator(mockConfig, mockBrowserProofValidator);
    });
    ...
    it('should return a score of 100 when validation is disabled', () => {
        ...
        const validator = new FingerprintValidator(disabledConfig, mockBrowserProofValidator);

        const result = validator.validate('validfingerprint', {});
    ...
    it('should return a lower score if some core components are missing', () => {
        const data: IClientFingerprint = {userAgent: 'test-agent'};
    ...
    it('should use the browser proof score if it is lower than the current score', () => {
        ...
        expect(mockBrowserProofValidator.validateBrowserProofs).toHaveBeenCalledWith(data.browserProofs);
    });
    ...
    it('should ensure the score is not above 100', () => {
        ...
        const validator = new FingerprintValidator(overlyHighScoreConfig, mockBrowserProofValidator);
        const result = validator.validate('validfingerprint', {});
